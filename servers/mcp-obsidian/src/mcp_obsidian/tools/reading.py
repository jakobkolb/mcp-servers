...
def _note_to_dict(
    ...
    return {
        "path": note.path,
        "frontmatter": note.frontmatter if include_frontmatter else None,
        "content": content,
        "raw": note.raw,
        "mtime": note.mtime,
        "size": note.size,
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "include_content": {
                        "type": "boolean",
                        "default": True,
                        "description": "Set false to omit the body (frontmatter-only read).",
                    },
                    "include_frontmatter": {
                        "type": "boolean",
                        "default": True,
                        "description": "Set false to omit frontmatter (body-only read).",
                    },
                },
                "required": ["path"],
            },
...
def get_handlers(config: Config) -> dict[str, Callable[..., Any]]:
    async def handle_read_note(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        return _note_to_dict(
            note, args.pretty_print, args.include_content, args.include_frontmatter
        )
